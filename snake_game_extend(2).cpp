...
LedControl lc = LedControl(11, 13, 10, 2);  //ledControl(din,clk,cs,numberofDevices);
void snakeMove();
void printSnake();
void snakeHed();
int buttonPress();
...
void setup() {
  ...
  randomSeed(analogRead(A0)); //this will create more randomnes to randomnes function 
}
...
void printSnakeFood(){ //This print's the food on display
  if(snakeFoodY <8){
    lc.setLed(0,snakeFoodY,snakeFoodX,1);

  }
  else if(snakeFoodY>7){
    int fy2 = snakeFoodY -8;
    lc.setLed(1,fy2,snakeFoodX,1);
  }
}
...
void snakeHed(){    // controls the forward movement of the snake hed 
  if(snakeDirY == 1){
    snakeHedY++;
    if(snakeHedY>15) snakeHedY=0;
  }
  else if(snakeDirY == -1){
    snakeHedY--;
    if(snakeHedY <0) snakeHedY = 15; //change
  }
  else if(snakeDirX == 1){ 
    snakeHedX++;
    if(snakeHedX>7) snakeHedX = 0; /// change
  }
  else if(snakeDirX == -1){
    snakeHedX--;
    if(snakeHedX<0) snakeHedX = 7;  /// change
  }
}
